...
export default function PassChange() {
  ...
  useEffect(() => {
    const dato = sessionStorage.getItem('email_reset');
    if (dato) {
      setIdentificador(dato);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    try{
      ...
      if (password_nueva == password_actual) {
        toast.error('La nueva contraseña tiene que ser distinta a la anterior');
        return;
      }

      const data = await auth_api.changePassword(identificador, password_actual, password_nueva, confirmar_password_nueva);
      if(data.data){
